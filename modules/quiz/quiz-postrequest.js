module.exports = app => {
  ...
  app.post("/quiz", async function(req, res) {
    const sportQuizRequest = await fetch(
    ...
    sportQuizData.map(x => {
      ...
      const groupCalc = (() => {
        const groupOrSoloInObject =
          x.groupOrSolo === yourResultsOfForm.groupOrSolo
        ...
        if (groupOrSoloInObject === false) {
          const calcedPercentage =
            33.33 -
            Math.abs(yourResultsOfForm.groupOrSolo - x.groupOrSolo) * 3.33
          percentage += calcedPercentage
        }
      })()

      const inOutdoorCalc = (() => {
        const inOrOutdoorInObject =
          x.inOrOutdoor === yourResultsOfForm.inOrOutdoor
        ...
        if (inOrOutdoorInObject === false) {
          const calcedPercentage =
            33.33 -
            Math.abs(yourResultsOfForm.inOrOutdoor - x.inOrOutdoor) * 3.33
          percentage += calcedPercentage
        }
      })()
